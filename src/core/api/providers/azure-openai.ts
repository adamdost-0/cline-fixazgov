...
interface AzureOpenAiHandlerOptions extends CommonApiHandlerOptions {
	openAiApiKey?: string
...
export class AzureOpenAiHandler implements ApiHandler {
	private options: AzureOpenAiHandlerOptions
	private client: AzureOpenAI | undefined

	constructor(options: AzureOpenAiHandlerOptions) {
	...
	private ensureClient(): AzureOpenAI {
		if (!this.client) {
			...
			if (!this.options.openAiApiKey && !this.options.azureIdentity) {
				throw new Error("Azure OpenAI API key or Azure Identity Authentication is required")
			}
			...
			try {
				...
				throw new Error(`Error creating Azure OpenAI client: ${error.message}`)
			}
		}
		return this.client
	}
