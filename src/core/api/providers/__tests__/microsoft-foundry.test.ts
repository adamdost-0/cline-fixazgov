...
describe("Microsoft Foundry Provider", () => {
	...
	describe("Token Scope Selection", () => {
		it("should return commercial scope for commercial endpoints", () => {
			getAzureCognitiveScope("https://test.openai.azure.com/")?.should.equal("https://cognitiveservices.azure.com/.default")
		...
		it("should return government scope for government endpoints", () => {
			getAzureCognitiveScope("https://test.openai.azure.us/")?.should.equal("https://cognitiveservices.azure.us/.default")
			getAzureCognitiveScope("https://test.usgovcloudapi.net/")?.should.equal("https://cognitiveservices.azure.us/.default")
		})
		...
		describe("Handler Initialization", () => {
			it("should throw error when endpoint is missing", async () => {
				...
				try {
					...
					thrown = true
					;(error as Error).message.should.match(/endpoint URL is required/i)
				}
				thrown.should.equal(true) // Ensure error was thrown above
			})
			...
			it("should throw error when deployment name is missing", async () => {
				...
				try {
					...
					thrown = true
					;(error as Error).message.should.match(/deployment name is required/i)
				}
				thrown.should.equal(true) // Ensure error was thrown above
			})
